...
pub fn bert() {
    ...
    let actual_tokens = vec![101, 1996, 3007, 1997, 2605, 2003, 1026, 7308, 1028, 1012, 102];
    let mut attention_mask = vec![0; 1 * 32];
    for (i, v) in actual_tokens.iter().enumerate() {
        input_tokens[i] = *v;
        attention_mask[i] = 1;
    }
    let input = HashMap::from([(0 as usize, TensorValue::Int64(
        ArrayD::from_shape_vec(vec![1, 32], input_tokens).unwrap(),
    )), (
        1 as usize, TensorValue::Int64(
            ArrayD::from_shape_vec(vec![1, 32], attention_mask).unwrap(),
        )
    )]);
    let result = resnet.compute(&input).unwrap();
...
pub fn simple_bert() {
    ...
    let input = HashMap::from([(0 as usize, TensorValue::Int64(
        ArrayD::from_shape_vec(vec![1, 4], input_tokens).unwrap(),
    )), (
        1 as usize, TensorValue::Int64(
            ArrayD::from_shape_vec(vec![1, 4], vec![1, 0, 1, 1]).unwrap(),
        )
    )]);
    let result = resnet.compute(&input).unwrap();
