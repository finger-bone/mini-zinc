...
pub fn bert() {
    ...
    let mut input_tokens = vec![0; 1 * 482];
    ...
    let mut attention_mask = vec![0; 1 * 482];
    for (i, v) in actual_tokens.iter().enumerate() {
        input_tokens[i] = *v;
        attention_mask[i] = 1;
    }
    let input = HashMap::from([(0 as usize, TensorValue::Int64(
        ArrayD::from_shape_vec(vec![1, 482], input_tokens).unwrap(),
    )), (
        1 as usize, TensorValue::Int64(
            ArrayD::from_shape_vec(vec![1, 482], attention_mask).unwrap(),
        )
    )]);
    let result = resnet.compute(&input).unwrap();
