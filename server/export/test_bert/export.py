...
class SimpleTransformer(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        def reshape_heads(tensor):
            ...
            # return tensor.permute(0, 2, 1, 3)  # [B, nh, T, hd]
...
dummy_input = torch.tensor([[1, 0, 1, 2]], dtype=torch.long)  # [1, 4]
dummy_attention_mask = (dummy_input != 0).to(dtype=torch.long)  # padding mask, True表示有效

# 注意 jit.trace 需要两个参数一起传入
traced = torch.jit.trace(model, (dummy_input, dummy_attention_mask))

traced.save("test_bert.pt")